/* Job report template (jobReport() in the prototype). The browser prints it; /api/board/report renders the same
   HTML to PDF on the server for Supabase Storage and the property manager (INTEGRATION.md §10). */
import { priceOf } from "@/lib/domain/health";
import { dur, fmtDate, fmtDay, jobNo, money, winTxt } from "@/lib/domain/time";
import { EXPECT, SERVICES, type Job, type Prop, type Team } from "@/lib/domain/types";
import { strings, type Lang } from "@/lib/i18n";
import { ME } from "@/lib/operators";

const esc = (s: unknown) => String(s ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c]!);

/** Placeholder evidence photo (the prototype's photoSrc), used until Job Tracker photo URLs exist. */
...
export interface ReportInput {
  ...
  team: Team | null;
  lang: Lang;
  /** Real evidence URLs (live mode); placeholders from the evidence count otherwise. */
  photos?: { before: string[]; after: string[] };
}

export function reportHtml({ job: j, prop: p, custName, team, lang, photos }: ReportInput) {
  const l = strings(lang), T2 = l.st2, pr = priceOf(j, p), m = pr.price - pr.pay, exp = EXPECT[j.svc];
  ...
  const urls = { before: photos?.before ?? Array.from({ length: pre }, (_, i) => photoSrc("before", i)), after: photos?.after ?? Array.from({ length: post }, (_, i) => photoSrc("after", i)) };
  const shots = (kind: "before" | "after") => (urls[kind].length ? urls[kind].map((u) => `<img src="${esc(u)}">`).join("") : `<span class="mut">&mdash;</span>`);
  ...
  <h2>${esc(T2.evidence)}</h2><p><b>${esc(T2.before)}</b><br>${shots("before")}</p><p><b>${esc(T2.after)}</b><br>${shots("after")}</p>
  ...
  return `<!DOCTYPE html><html lang="${lang}"><head><meta charset="utf-8"><title>jobRUN ${jobNo(j)}</title><style>${css}</style></head><body>${body}</body></html>`;
}
